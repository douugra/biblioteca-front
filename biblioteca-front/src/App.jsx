  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6">
      ...
      <form onSubmit={addLivro} className="bg-white p-4 rounded-2xl shadow-md flex gap-3 mb-6">
        <input type="text" placeholder="Título" value={titulo} onChange={e => setTitulo(e.target.value)} className="border p-2 rounded-lg" />
        <input type="text" placeholder="Autor" value={autor} onChange={e => setAutor(e.target.value)} className="border p-2 rounded-lg" />
        <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">Adicionar</button>
      </form>
          ...
          <ul className="divide-y">
            {livros.map(l => (
              <li key={l.id} className="flex justify-between items-center py-2">
                ...
                <button onClick={() => deleteLivro(l.id)} className="bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600">Remover</button>
              </li>
            ))}
          </ul>
        )}
